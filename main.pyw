...
class Login_window:
	def __init__(self, root):
		...
		#----------register button----------
		button_register = Button(self.root, command=self.redirect_admin, text="Register New Account?", cursor="hand2", bg="black", fg="red", bd=0, font="georgia 12").place(x=600,y=460)
	...
	def forget_password(self):
		if self.combo_question.get()=="Select" or self.answer_entry.get()=="" or self.newpass_entry.get()=="":
			...
			try:
				mydb = pymysql.connect(user='root', host='localhost', password='', database='AI_Surveillance_System')
				...
				messagebox.showerror("Error",f"Error due to : {str(es)}", parent=self.new_root)
	...
	def login(self):
		if self.email_entry.get()=="" or self.pass_entry.get()=="":
			...
			try:
				mydb = pymysql.connect(user='root', host='localhost', password='toor', database='AI_Surveillance_System')
				mycursor = mydb.cursor()
